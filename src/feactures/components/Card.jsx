import React from "react";

const Card = (props) => {
  const { year, mileage, brand, image, monthly_installment } = props;

  return (
    <article className="    w-80 mt-8 flex flex-col justify-between  rounded-xl shadow-xl">
      <div className=" h-[18rem] rounded-t-xl overflow-hidden">
        <figure className="h-full relative">
          <img
            className="car-img  h-full w-full object-cover object-center "
            alt="carros"
            src={image}
          />
          
          <div>
            <img
              src="data:image/png;base64,"
              alt="Imagen Normal"
              class=" absolute top-0 right-4 h-12  hover:top-3"
            ></img>
            <div className="bg-tertiary text-transparent absolute top-0 right-4 w-[3.51rem]  border-l-[1.34px] border-r-[1.34px] border-white transition duration-700 hover:border-r-5  hover:bg-purple-400">
              {"as"}
            </div>
          </div>
        </figure>
      </div>

      <div className=" texto-1 spincar-srp-360-wa-button display: block; width: 100%; background-color: rgb(0, 161, 155); text-align: center; font-weight: bold; cursor: pointer; z-index: 1">
        360° WalkAround
      </div>

      <div className="datos p-4 flex justify-between">
        <div>
          <div className="flex flex-wrap gap-1 text-lg font-semibold md:text-xl">
            <h2 className="capitalize">{brand}</h2>
          </div>
          <span className="block text-gray-800 text-md md:text-lg mt-2">
            {`${year} | ${mileage} km`}
          </span>
          <span className="block text-gray-800 text-md md:text-lg mt-2">
            Cuota mensual:
          </span>
          <span className="block text-gray-800 text-md md:text-lg mt-2">
            {` ${
              monthly_installment ? `$${monthly_installment}` : "No hay cuota"
            }`}
          </span>
          <div className="flex flex-col gap-1 justify-end">
            <a
              className="bg-purple-400 inline-flex justify-center items-center rounded-lg border disabled:opacity-70 disabled:cursor-not-allowed focus:outline-none hover:opacity-80 focus:shadow-none focus:opacity-[0.85] active:shadow-none active:opacity-[0.85] disabled:shadow-none transition-opacity bg-primary text-white bg-gradient-to-r from-tertiary to-primary py-2 px-3 text-md !p-1"
              href="/"
            >
              <span className="mx-2">Ver detalles</span>
            </a>
          </div>
        </div>
      </div>
    </article>
  );
};

export default Card;
